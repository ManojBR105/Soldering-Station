...
const byte buzzerPIN = 13;                      // The simple buzzer to make a noise
...
class IRON_CFG : public CONFIG {
  ...
  private:
    ...
    uint16_t t_tip[3];
    const uint16_t def_tip[3] = {420, 660, 885};// Default values of internal sensor readings at reference temperatures
    const uint16_t def_set = 513;               // Default preset temperature in internal units
    const uint16_t ambient_temp  = 18;         // Ambient temperatire in the internal units
...
class DSPL : protected LiquidCrystal {
  ...
  const byte cent[8] = {B01110, B01010, B01110, B00000, B00000, B00000, B00000};
};

void DSPL::init(void) {
  LiquidCrystal::begin(16, 2);
  LiquidCrystal::clear();
  LiquidCrystal::createChar(1, cent);
...
class IRON : protected PID {
  ...
  private:
    ...
    const byte     max_power       = 120;       // maximum power to the iron (220)
    const byte     max_fixed_power = 180;       // Maximum power in fiexed power mode
